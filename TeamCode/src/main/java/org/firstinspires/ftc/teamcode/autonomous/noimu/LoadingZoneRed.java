...
@Disabled
@Autonomous
public class LoadingZoneRed extends HydrofluoricLinearOpMode {
    ...
    public void runOpMode(){
        ...
        while(!isStarted()) {
            if (skystoneDetectorBlue.valLeft == 0) skystonePosition = 2;
            else if (skystoneDetectorBlue.valMid == 0) skystonePosition = 1;
        ...
        if(skystonePosition == 0) {
            ...
            encoderTurn(-RobotConstants.FIRST_TURN_DIST, 0.3);
            //drive under skybridge and release skystone
            driveToPosition(48.6, 0.5);
            ...
            driveToPosition(20, 0.3);
            //drive back
            driveToPosition(-18, 0.3);
            robot.intake.off();
            //turn towards skybridge
            encoderTurn(-RobotConstants.SECOND_TURN_DIST, 0.3);
            //drive under skybridge and release skystone
            driveToPosition(70, 0.5);
            robot.intake.reverse();
            sleep(500);
            //park
            driveToPosition(-21.2, 0.5);
            robot.intake.off();
        }
        ...
        strafeToPosition(15,SLOW_SPEED);
        encoderTurn(180, SLOW_SPEED);
        ...
        /*driveToPosition(RobotConstants.QUARRY_2_DRIVE,SLOW_SPEED);

        //turn towards leftmost stone
        encoderTurn(RobotConstants.QUARRY_3_TURN, SLOW_SPEED);

        //intakes left stone
        robot.intake.on();
        driveToPosition(RobotConstants.QUARRY_4_INTAKE,SLOW_SPEED);
        robot.intake.off();
        driveToPosition(RobotConstants.QUARRY_5_INTAKEREVERSE,SLOW_SPEED);

        telemetry.addData("Heading: ", robot.getHeading());

        //turns and moves under skybridge
        encoderTurn(-RobotConstants.QUARRY_3_TURN, SLOW_SPEED);
        driveToPosition(RobotConstants.QUARRY_6_ZOOM,FAST_SPEED);
        ...
        //zooms back
        driveToPosition(RobotConstants.QUARRY_7_ZOOM_BACK,FAST_SPEED);

        //picks up second stone
        encoderTurn(RobotConstants.QUARRY_8_TURN2, SLOW_SPEED);
        robot.intake.on();
        driveToPosition(RobotConstants.QUARRY_9_INTAKE2, SLOW_SPEED);
        driveToPosition(RobotConstants.QUARRY_10_INTAKE2REVERSE, SLOW_SPEED);
        robot.intake.off();

        encoderTurn(-RobotConstants.QUARRY_8_TURN2, SLOW_SPEED);
        driveToPosition(RobotConstants.QUARRY_11_ZOOM2,FAST_SPEED);
        robot.intake.reverse();
        sleep(500);
        robot.intake.off();

        driveToPosition(RobotConstants.QUARRY_12_PARK,FAST_SPEED);*/
    }
}
